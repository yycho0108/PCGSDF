...
inline __device__ int2 make_int2(int s) { return make_int2(s, s); }
...
inline __device__ uint2 make_uint2(uint s) { return make_uint2(s, s); }
...
inline __device__ int3 make_int3(int s) { return make_int3(s, s, s); }
...
// inline __device__ float3 fxyz(float4 a)
//{
//  return make_float3(float(a.x), float(a.y), float(a.z));
//}
...
inline __device__ float2 operator/(const float2 a, const int2 b) {
    return make_float2(a.x / b.x, a.y / b.y);
}
...
inline __device__ float2 max(float2 a, float2 b) {
...
inline __device__ float max(const float2& a){
    return max(a.x,a.y);
}

inline __device__ float max(const float3& a){
    return max(max(a.x,a.y),a.z);
}
inline __device__ float3 max(const float3& a, float b){
    return make_float3(max(a.x,b), max(a.y,b), max(a.z,b));
}
inline __device__ float2 max(const float2& a, float b){
...
inline __device__ uint clamp(uint f, uint a, uint b) {
    return max(a, min(f, b));
}
...
inline __device__ float3 clamp(float3 v, float3 a, float3 b) {
...
inline __device__ float4 clamp(float4 v, float4 a, float4 b) {
    return make_float4(clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y),
            clamp(v.z, a.z, b.z), clamp(v.w, a.w, b.w));
}

inline __device__ int2 clamp(int2 v, int a, int b) {
    return make_int2(clamp(v.x, a, b), clamp(v.y, a, b));
}
inline __device__ int2 clamp(int2 v, int2 a, int2 b) {
    return make_int2(clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y));
}
inline __device__ int3 clamp(int3 v, int a, int b) {
    return make_int3(clamp(v.x, a, b), clamp(v.y, a, b), clamp(v.z, a, b));
}
inline __device__ int3 clamp(int3 v, int3 a, int3 b) {
    return make_int3(clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y),
            clamp(v.z, a.z, b.z));
}
inline __device__ int4 clamp(int4 v, int a, int b) {
    return make_int4(clamp(v.x, a, b), clamp(v.y, a, b), clamp(v.z, a, b),
            clamp(v.w, a, b));
}
inline __device__ int4 clamp(int4 v, int4 a, int4 b) {
    return make_int4(clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y),
            clamp(v.z, a.z, b.z), clamp(v.w, a.w, b.w));
}

inline __device__ uint2 clamp(uint2 v, uint a, uint b) {
    return make_uint2(clamp(v.x, a, b), clamp(v.y, a, b));
}
inline __device__ uint2 clamp(uint2 v, uint2 a, uint2 b) {
    return make_uint2(clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y));
}
inline __device__ uint3 clamp(uint3 v, uint a, uint b) {
    return make_uint3(clamp(v.x, a, b), clamp(v.y, a, b), clamp(v.z, a, b));
}
inline __device__ uint3 clamp(uint3 v, uint3 a, uint3 b) {
    return make_uint3(clamp(v.x, a.x, b.x), clamp(v.y, a.y, b.y),
            clamp(v.z, a.z, b.z));
}
inline __device__ uint4 clamp(uint4 v, uint a, uint b) {
    return make_uint4(clamp(v.x, a, b), clamp(v.y, a, b), clamp(v.z, a, b),
            clamp(v.w, a, b));
}
inline __device__ uint4 clamp(uint4 v, uint4 a, uint4 b) {
...
inline __device__ float length(const float2 v) { return sqrtf(dot(v, v)); }
inline __device__ float length(const float3 v) { return sqrtf(dot(v, v)); }
inline __device__ float length(const float4 v) { return sqrtf(dot(v, v)); }

inline __device__ float length(const float v) { return abs(v); }
inline __device__ float length(const float x, const float y) { return sqrtf(x*x+y*y); }
inline __device__ float length(const float x, const float y,
        const float z) { return sqrtf(x*x+y*y+z*z); }
...
inline __device__ float fracf(float v) { return v - floorf(v); }
...
inline __device__ float2 abs(float2 v) {
...
// cross product
////////////////////////////////////////////////////////////////////////////////
inline __device__ float cross(const float2 a, const float2 b){
...
inline __device__ float2 smoothstep(float2 a, float2 b, float2 x) {
    float2 y = clamp((x - a) / (b - a), 0.0f, 1.0f);
    return (y * y * (make_float2(3.0f) - (make_float2(2.0f) * y)));
}
inline __device__ float3 smoothstep(float3 a, float3 b, float3 x) {
    float3 y = clamp((x - a) / (b - a), 0.0f, 1.0f);
    return (y * y * (make_float3(3.0f) - (make_float3(2.0f) * y)));
}
inline __device__ float4 smoothstep(float4 a, float4 b, float4 x) {
    float4 y = clamp((x - a) / (b - a), 0.0f, 1.0f);
    return (y * y * (make_float4(3.0f) - (make_float4(2.0f) * y)));
}

// rotate
inline __device__ float3 rotate(const float4 q, const float3 v) {
    float vx = q.y*v.z-q.z*v.y;
    float vy = q.z*v.x-q.x*v.z;
    float vz = q.x*v.y-q.y*v.x;
    vx += vx;
    vy += vy;
    vz += vz;
    return make_float3(
            v.x + q.w*vx + q.y*vz-q.z*vy,
            v.y + q.w*vy + q.z*vx-q.x*vz,
            v.z + q.w*vz + q.x*vy-q.y*vx
            );
}

// sign
inline __device__ float sign(const float x){
    return (x>0) - (x<0);
}
