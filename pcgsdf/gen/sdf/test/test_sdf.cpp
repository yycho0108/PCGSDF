...
Eigen::MatrixXf CreateDepthImage(const Eigen::Isometry3f &camera_pose,
  ...
  auto sdf = cho::gen::OpTransformation::Create(scene, camera_pose.inverse());
...
cho::gen::SdfPtr TrajectoryToSdf(
  ...
  for (auto it = std::next(it_prv); it != trajectory.end(); ++it) {
    ...
    sdf = cho::gen::OpTransformation::Create(sdf, Eigen::Translation3f{t} * q);
...
int main() {
  ...
  cho::gen::SdfPtr free_space = cho::gen::OpSubtraction::Create(space, objs);
